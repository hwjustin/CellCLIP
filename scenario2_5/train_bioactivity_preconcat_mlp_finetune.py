"""Train a 209-task bioactivity classifier with pre-concat projection MLP ablation.

Scenario 2.5 pipeline:
1) Load pretrained CellCLIP checkpoint.
2) Build per-well bags of site embeddings and perturbation text tokens.
3) Fine-tune image encoder + text encoder + per-modality projection MLPs + classifier.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Jointly fine-tune CellCLIP with pre-concat per-modality projection MLPs "
            "for 209-task bioactivity prediction."
        )
    )
    parser.add_argument(
        "--ckpt_path",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional override LR for projection MLPs. Defaults to head_lr.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Set CellCLIP backbone LR to 0 for first N epochs.",
    )
    parser.add_argument(
        "--embed_dim",
        type=int,
        default=512,
        help="Output width for modality-specific projection MLPs before concat.",
    )
    parser.add_argument(
        "--proj_hidden_dim",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use residual projection (x + MLP(x)) when input_dim == embed_dim.",
    )
    parser.add_argument(
        "--proj_identity_init",
        action="store_true",
        default=False,
        help=(
            "Initialize projection MLPs as identity when possible. "
            "Implemented as zero-initialized residual delta, requiring input_dim == embed_dim."
        ),
    )
    parser.add_argument(
        "--proj_dropout",
        type=float,
        default=0.0,
        help="Dropout applied on each projected branch output.",
    )
    parser.add_argument(
        "--fc_units",
        type=int,
    ...
    parser.add_argument(
        "--out_dir",
        type=str,
        default="results/bioactivity_preconcat_mlp_finetune",
...
class ProjectionMLP(nn.Module):
    """Scenario3-style projection MLP for each modality before concat."""
    ...
    def __init__(
        ...
        if num_layers == 1:
            ...
            raise ValueError(f"Unsupported proj_num_layers={num_layers}, expected 1 or 2.")
        self.proj_dropout = nn.Dropout(p=proj_dropout) if proj_dropout > 0 else nn.Identity()
        can_residual = input_dim == output_dim
        # Identity init needs residual form (x + delta(x)) to be exact at step 0.
        self.use_residual = (residual or self.identity_init) and can_residual
    ...
    def _init_parameters(self):
        if self.identity_init:
            if self.input_dim != self.output_dim:
                raise ValueError(
                    "proj_identity_init requires input_dim == output_dim "
                    f"(got {self.input_dim} vs {self.output_dim})."
                )
            # Exact identity at init: out = x + 0, with residual enabled above.
...
class CellPaintSSLMLPHead(nn.Module):
    ...
    def __init__(
        ...
        if num_hidden_layers < 1:
            raise ValueError("cls_num_hidden_layers must be >= 1.")

        layers: List[nn.Module] = [nn.Dropout(p=cls_input_dropout)]
        in_dim = input_dim
        for _ in range(num_hidden_layers):
            layers.extend(
                [
                    nn.Linear(in_dim, fc_units),
                    nn.ReLU(inplace=True),
...
class PreConcatMLPJointBioactivityModel(nn.Module):
    def __init__(
        self,
        ...
        proj_identity_init: bool,
        proj_dropout: float,
        classifier: nn.Module,
    ):
        super().__init__()
        self.cellclip_backbone = cellclip_backbone
        self.cell_proj = ProjectionMLP(
            cell_feature_dim,
            embed_dim,
            ...
            identity_init=proj_identity_init,
        )
        ...
        self.concat_norm = nn.LayerNorm(2 * embed_dim)
    ...
    def forward(self, imgs: torch.Tensor, mol: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        text_feats = self.cellclip_backbone.encode_text(mol)
        cell_proj = self.cell_proj(cell_feats)
        text_proj = self.text_proj(text_feats)
        feats = torch.cat([cell_proj, text_proj], dim=1)
...
def build_param_groups(
    model: PreConcatMLPJointBioactivityModel,
    backbone_lr: float,
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if name.startswith("cellclip_backbone."):
            back_name = name[len("cellclip_backbone.") :]
...
def infer_feature_dims(backbone, loader, device: str) -> Tuple[int, int]:
    backbone.eval()
...
def main():
    ...
    cell_feature_dim, text_feature_dim = infer_feature_dims(
        backbone=backbone, loader=train_loader, device=device
    )
    print(
        f"[Info] Inferred feature dims: cell={cell_feature_dim}, text={text_feature_dim}, "
        f"projected={args.embed_dim}"
    )
    ...
    classifier = CellPaintSSLMLPHead(
        ...
        num_hidden_layers=args.cls_num_hidden_layers,
    ...
    if args.freeze_backbone_epochs > 0:
        print(f"[Info] Freeze backbone for first {args.freeze_backbone_epochs} epochs.")
    if args.proj_lr is not None:
        print(f"[Info] Projection LR override enabled: proj_lr={args.proj_lr:.2e}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Keep explicit param-group assignment for reproducibility with scenario3-style grouping.
    ...
    ckpt_out_path = os.path.join(args.out_dir, "best_preconcat_mlp_joint_model.pt")
    torch.save(
        {
            ...
            "fc_units": args.fc_units,
            "cls_num_hidden_layers": args.cls_num_hidden_layers,
            "cls_input_dropout": args.cls_input_dropout,
            "cls_dropout": args.cls_dropout,
